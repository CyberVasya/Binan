    ...
    if(len(rs['result'])>0):
        ...
        if(time.time()-datet)<telegram_delay:
            ...
            if 'exit' in textt:
                exit()
            if 'hello' in textt:
                telegram_bot_sendtext('Привет. Вывожу для тебя ...')
...
def main(step):
    ...
    try:
        ...
        prt('\n\nЧто то пошло не так. Запускаю заново...')
...
while time.time() <= timeout:
    try:
        prt('Продолжение запуска скрипта в' +time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
        main(counterr)
        counterr=counterr+1
        if counterr>5:
            counterr=1
